...
function createPostHtml(postData,largeFont=false) {
  ...
  //if(isRetweet && postData==null)console.log(old);
    ...
    if(postData.images.length!=0){
        imagePostShow=`
        <div class='ImagePostShow'>
        <img src="${postData.images[0].url}" alt="" id='imagePost' width='${width} height='${height}'>
        </div>
        `
    }
...
function messageReceived(newMessage) {
    if($(`[data-room="${newMessage.chat._id}"]`).length == 0){//user is not on tha chat page
